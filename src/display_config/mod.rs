...
impl ApplyLogicalMonitor {
    pub async fn from(logical_monitor: &LogicalMonitor,
...
impl ApplyConfig {
    pub async fn from(state: State) -> Self {
        let physical_monitor = state.physical_monitors
            .iter()
            .find(|pm| pm.is_builtin())
            .unwrap();

        let logical_monitor = state.logical_monitors
            .iter()
            .find(|lm| {
                lm
                    .monitors
        ...
        Self {
            ...
            logical_monitors: vec![ApplyLogicalMonitor::from(logical_monitor, physical_monitor).await],
            properties: HashMap::new(),
        }
    }
}
